<template>
  ...
  >
    <li class="page-item d-flex flex-grow-1 align-items-center">
      <button type="button" class="page-link" v-on:click="previousWeek">Previous</button>
    </li>
    <li class="page-item disabled d-flex flex-column" aria-current="page">
      <div class="d-flex justify-content-center page-link top-border">{{ (selectedWeek + currentWeek) == 52 ? selectedWeek + currentWeek : (selectedWeek + currentWeek) % 52 }}. hét</div>
      <div class="d-flex justify-content-center flex-nowrap flex-grow-1 page-link bottom-border">{{ datesOfWeek[0] }} - {{ datesOfWeek[6] }}</div>
    </li>
    <li class="page-item d-flex align-items-center">
      <button type="button" class="page-link" v-on:click="nextWeek">Next</button>
    </li>
  </ul>
</template>
